...
def __main__():
    ...
    logging.basicConfig(filename= config.drLogFile, level=logging.DEBUG, format=config.format, datefmt='%d-%m-%Y %H:%M')
    ...
    dbSqlObj = gdbi.data = gdbi.DbSql(dataIn)
    ...
    _, message = dbSqlObj.prepare_script(config.sqlPathName, vars=config.drSqlVarReplacements)
    ...
    for index, key in enumerate(devices):
        ...
        # create the report
        strToday = config.drSqlVarReplacements['dayPlaceHolder'].strip('\'')
    ...
    dataIn = {
        ...
        'html': '<html><body><p> Please find attached the daily report for Google non-brand PPC </p></body></html>',
